/**
 * fiat-tools —— L1 业务工具扩展（阶段 3/4/5 的只读、dry-run 与 apply 工具）。
 *
 ...
 *
 * 阶段 5 新增 apply 模式：`fiat_cashback_reconcile` 传 mode=apply 时，不直接写，而是
...
export function createFiatTools(deps: FiatToolsDeps) {
	return (pi: ExtensionAPI) => {
		...
		for (const def of FIAT_TOOLS) {
			...
			pi.registerTool(defineFiatTool(def, deps, applyTools));
		}
	};
}

function defineFiatTool(def: FiatToolDef, deps: FiatToolsDeps, applyTools: Set<string>) {
