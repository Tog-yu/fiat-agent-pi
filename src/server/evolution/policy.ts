...
const APPROVAL_BYPASS_PATTERNS: RegExp[] = [
	...
	// 目标的枚举里含「校验 / 验证」而非只有「审批 / 审核 / 复核」：
	// 阶段 15 §15.9 的禁写形态把「跳过校验」「免复核」与「无需审批」并列 ——
	// 三者是同一件事的三个说法（都是「把某道闸门掏空」），记忆与技能都不该承载。
	// 动词侧的 `(?<!避)免` 是给「免复核」「免审」这类简写留的口，而 `(?<!避)` 用来挡掉
	// 「避免审批超时」—— 那是个正经句子，不该被误杀。
	/(?:无需|不用|不必|跳过|绕过|(?<!避)免(?:除|去)?|豁免|取消)\s*(?:再)?\s*(?:人工)?\s*(?:审批|审核|复核|校验|验证|approval|approve|validation)/i,
...
/**
 * 正文归一化：降小写 + 去掉全部空白 / 标点 / 符号。
 *
 * 两个消费方共用它，且**必须共用**（P15-93 起）：技能 / 记忆的幂等键都定义为
 * `hash(target + 归一化正文)`，`DEV_SPEC.md` §15.6 约束 3 明确写了阶段 15
 * 与阶段 12 `applyProposal` **同口径**。两套归一化必然在「全角标点算不算」
 * 这类细节上分叉，而分叉的表现是「同一事实换个标点就重复入库」。
 */
